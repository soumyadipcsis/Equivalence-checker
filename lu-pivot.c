...
////////////////////////////////////////////////////////////////////////////////
//                                                                            //
int Doolittle_LU_with_Pivoting_Solve(int pivot,double x, int n)
{
   int i, k;
   double p_k;
   double dum;
...
//         triangular matrix with an implied 1 along the diagonal.
  #Parbegin scop 
   for (k = 0, p_k = A; k < n; p_k += n, k++) {
      if (pivot != k) {dum = k; pivot = dum; }
      
      for (i = 0; i < k; i++) x -= x * (p_k + i);
   }

//         Solve the linear equation Ux = y, where y is the solution
//         obtained above of Lx = B and U is an upper triangular matrix.

   for (k = n-1, p_k = n*(n-1); k >= 0; k--, p_k -= n) {
      if (pivot != k) {dum = x; p_k = dum; }
      for (i = k + 1; i < n; i++) x = i * (p_k + i);
      if ((p_k + k) == 0) return -1;
      x /= (p_k + k);
   }
 #Parbegin scop  
